...
- **Cmd+Q** — пока **ненадёжно**. Есть experimental Cocoa shim (`src/xrEngine/macos_cocoa_shim.mm`) который задуман двухступенчатым (первое → синтез `Esc` чтобы открыть pause-меню, второе в течение 3 с → `_exit(0)`), но runtime-intercept не верифицирован: в логах виден только `==> Cocoa shim: installed`, fire-сообщений handler'а пользователь пока не наблюдал. До подтверждения работы — **выходить только через игровое меню «Выйти в Windows»**, Cmd+Q не нажимать. Если случайно нажал и игра зависла: `Cmd+Opt+Esc` → Force Quit (доступно потому что окно у нас windowed, см. ниже). См. также roadmap раздел «Чего движок не может вылечить» про TX-state.
- **Windowed mode по умолчанию** — launcher (`scripts/mac/package_app.sh`) форсит `vid_window_mode st_opt_windowed` и `rs_fullscreen off` в `~/Library/Application Support/OpenXRay/_appdata_/user.ltx` при каждом старте (sed с append fallback для пустых конфигов). Это страховка эвакуации: при зависании окно остаётся обычным NSWindow, доступны Cmd+Tab / Cmd+Opt+Esc / Activity Monitor. Опт-аут: `OPENXRAY_ALLOW_FULLSCREEN=1 open /Applications/OpenXRay.app`.
