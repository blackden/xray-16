...
- **[P2, ✓ root cause найден 2026-05-27 — A.5 audit, #127] Пространственное аудио (EFX/HRTF) на macOS** — корень: `find_package(OpenAL)` в `cmake/XRay.Compiler.GNULike.cmake:160` резолвит Apple `/System/Library/Frameworks/OpenAL.framework` (deprecated с macOS 10.15), а не keg-only openal-soft из Brewfile. Apple framework не экспонирует ни EFX (`ALC_EXT_EFX`), ни HRTF — spatialization вырождается в простой amplitude panning. `snd_efx` cvar silently no-ops (открыт follow-up issue для лог-warning). Полная миграция на openal-soft — отдельный follow-up issue (требует `OPENAL_ROOT` injection в `if(APPLE)`, EFX backend C-side, dylib packaging). Аудит и detailed findings: [`notes/decisions/a5-audio-audit.md`](../decisions/a5-audio-audit.md).
