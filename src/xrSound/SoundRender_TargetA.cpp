...
#if __has_include(<efx.h>)
        // Route this source through the EFX aux slot so it contributes to
        // (and is wet-reverbed by) the global reverb effect. Without this
        // bind the slot stays silent regardless of effect parameters.
        if (pAuxSlot != 0)
