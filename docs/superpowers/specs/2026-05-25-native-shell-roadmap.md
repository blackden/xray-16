...
| **A.5** | Audio audit | ✓ **Готово (2026-05-27, #127, no code).** Гейт зелёный: `grep SDL_audio* по src/` = 0. SDL_Init поднимает только `SDL_INIT_VIDEO [\| SDL_INIT_GAMECONTROLLER]`. xrSound на OpenAL напрямую. Bonus finding: Apple's deprecated `OpenAL.framework` линкуется поверх keg-only openal-soft → нет EFX/HRTF на macOS, объясняет старый P2 «spatial audio works wrong». Детали: `notes/decisions/a5-audio-audit.md`. | grep `SDL_audio` / `SDL_OpenAudioDevice` в коде — пусто на macOS gate |
| **A.6** | Зачистка остатков (V4 contract) | ✓ **Готово (2026-05-27).** A.6.1 NSPasteboard clipboard (#137), A.6.2 NSAlert dialogs (#138), A.6.3 `ITextInputBackend` engine-owned contract с SDL impl (#139). Bonus root-fix `nsevent_input=1` default через monitor gate (#136), stuck modifier (#135), ring buffer atomics (#133). `sysctlbyname` / NSOpenPanel — 0 хитов в коде, dropped из scope. Native macOS text-input impl через `NSTextInputClient` — отложено до A.1 part 2 unparking. Детали: `notes/decisions/a6-textinput-contract.md`. | `SDL_*` calls под Apple-gate routed через engine abstractions с documented platform reason; `SDL_StartTextInput` survives behind `ITextInputBackend` до A.1 part 2 unparking |
